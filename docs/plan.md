...
**42 hand-verified questions** in `eval/golden_set.json`, evenly split between
`exact_term` (naming identifiers like `UnknownVersionInteroperabilityProxy`) and
`paraphrase` (plain language, deliberately avoiding the vocabulary), spanning 38
of the 56 documents.
...
18 were rejected: link-list chunks with no explanation, question/chunk
